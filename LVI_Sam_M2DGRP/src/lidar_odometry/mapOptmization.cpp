...
public:
    ...
    ros::Publisher pubCornerSurround;
    ros::Publisher pubSurfSurround;
    ...
    std::deque<nav_msgs::Odometry> vinsQueue;
    ...
    mapOptimization()
    {
        ...
        pubCornerSurround = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/lidar/mapping/map_corner", 1);
        pubSurfSurround = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/lidar/mapping/map_surf", 1);
        pubOdomAftMappedROS = nh.advertise<nav_msgs::Odometry>(PROJECT_NAME + "/lidar/mapping/odometry", 1); //原来的全局里程计
        pubPath = nh.advertise<nav_msgs::Path>(PROJECT_NAME + "/lidar/mapping/path", 1);//发布可视化
    ...
    {
        vinsQueue.push_back(*vinsMsg);
        if (first_lidar_keypose)
        {
            duration = ros::Time::now() - pre_lidar_time;
        }
        if (duration.toSec() > 1.0)
        {
            timeLastProcessing = vinsQueue.back().header.stamp.toSec();
            // 1.计算点云的先验位姿 (通过imu或者vins odom)
            timeLaserInfoStamp = vinsQueue.back().header.stamp;
            updateInitialGuess_vins();
            // 2.将scan to map匹配到的位姿与imu进行融合
            transformUpdate();
            // 3.保存关键帧, 更新因子图, 因子图优化 (odom gps loop因子)
            saveKeyFramesAndFactor();
            // 4.更新所有关键帧的位姿
            correctPoses();
            // 5.发布 lidar里程计 以及 tf:odom_to_lidar
            publishOdometry();
            // 6.发布 所有关键帧的位姿, localmap的surface点云, 当前帧点云(采样后的特征点云, 原始点云cloud_deskewed), 整条里程计轨迹globalPath
            publishFrames_vins();

            use_vins = true;

            cout << "using vins:" << ros::Time::now() << endl;
        }
    }
    void updateInitialGuess_vins()
    {
        nav_msgs::Odometry odomCur;
        odomCur = vinsQueue.back();
        //获取的是imu的位姿
        tf::Quaternion q_odom_imu;
        tf::quaternionMsgToTF(odomCur.pose.pose.orientation, q_odom_imu);
        tf::Transform t_odom_imu = tf::Transform(q_odom_imu, tf::Vector3(odomCur.pose.pose.position.x, odomCur.pose.pose.position.y, odomCur.pose.pose.position.z));
        ...
        tf::Transform t_odom_lidar = t_odom_imu * (t_lidar_imu.inverse());
        tf::Quaternion q_odom_lidar = tf::createQuaternionFromRPY(0, 0, 0) * (t_odom_lidar.getRotation());
        // t_odom_lidar = tf::Transform(q_odom_lidar, tf::Vector3(t_odom_lidar.getOrigin().x(), t_odom_lidar.getOrigin().y(), t_odom_lidar.getOrigin().z()));
        tf::Matrix3x3 ml(q_odom_lidar);
        double roll, pitch, yaw;
        ml.getRPY(roll, pitch, yaw);
        transformTobeMapped[3] = t_odom_lidar.getOrigin().x();
        transformTobeMapped[4] = t_odom_lidar.getOrigin().y();
        transformTobeMapped[5] = t_odom_lidar.getOrigin().z();
        transformTobeMapped[0] = roll;
        transformTobeMapped[1] = pitch;
        transformTobeMapped[2] = yaw;
    }
    void publishFrames_vins()
    {
    ...
    void publishGlobalMap()
    {
        ...
        pcl::PointCloud<PointType>::Ptr globalCornerMapKeyFrames(new pcl::PointCloud<PointType>());
        pcl::PointCloud<PointType>::Ptr globalSurfMapKeyFrames(new pcl::PointCloud<PointType>());
    ...
    void performLoopClosure(const std_msgs::Float64MultiArray &loopMsg)
    {
        ...
        {
            // get gtsam pose
            gtsam::Pose3 poseFrom = affine3fTogtsamPose3(Eigen::Affine3f(icp.getFinalTransformation()) * pose_diff_t * pose_cur); //此处的仿射变换写在左边
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void scan2MapOptimization() //经过icp之后已经获得了更新后的transformTobeMapped step4
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            for (int iterCount = 0; iterCount < 30; iterCount++)
            {
                ...
                if (LMOptimization(iterCount) == true) //经过优化后的位姿
                    break;
            }
    ...
    void transformUpdate() //优化后具有时延 所以用当前时刻的imu信息来进行优化
    {
        if (cloudInfo.imuAvailable == true)
        {
            if (std::abs(cloudInfo.imuPitchInit) < 1.4)
            {
                ...
                tf::Matrix3x3(transformQuaternion.slerp(imuQuaternion, imuWeight)).getRPY(rollMid, pitchMid, yawMid); //四元数之间球面插值得到的结果
    ...
    bool saveFrame()
    {
        ...
        Eigen::Affine3f transStart = pclPointToAffine3f(cloudKeyPoses6D->back()); //上一时刻塞进来的关键帧
    ...
    void publishOdometry()
    {
        ...
        br.sendTransform(trans_odom_to_lidar);//发布至TF树中
    }
    ...
    void publishFrames()
    {
        ...
        if (pubPath.getNumSubscribers() != 0)
        {
            ...
            globalPath.header.frame_id = "odom";
            pubPath.publish(globalPath);
            // cout<<"globalPath"<<globalPath.header.stamp<<endl;
        }
    }
};
