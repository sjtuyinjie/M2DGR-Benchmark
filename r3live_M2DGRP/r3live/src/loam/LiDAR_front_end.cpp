...
int main( int argc, char **argv )
{
    ...
    if(n.getParam("r3live_lio/lidar_imu_r", lidar_imu_rotm)){
        lidar_rot << lidar_imu_rotm[0], lidar_imu_rotm[1], lidar_imu_rotm[2], lidar_imu_rotm[3],
                 lidar_imu_rotm[4], lidar_imu_rotm[5], lidar_imu_rotm[6], lidar_imu_rotm[7],
                 lidar_imu_rotm[8], lidar_imu_rotm[9], lidar_imu_rotm[10], lidar_imu_rotm[11],
                 0.0, 0.0, 0.0, 1.0;
    }else{
...
void rs32_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    //add time
        ...
        float startOrientation = -atan2(pl_orig.points[0].y, pl_orig.points[0].x);
        float endOrientation   = -atan2(pl_orig.points[pl_orig.points.size() - 1].y,
                                                     pl_orig.points[pl_orig.points.size() - 1].x) + 2 * M_PI;
    ...
    for ( int i = 0; i < pl_orig.points.size(); i++ )
    {
        ...
        //add time
        float ori = -atan2(pl_orig.points[i].y, pl_orig.points[i].x);
            ...
            if( i == 0) relTime0 = relTime;
            

        added_pt.curvature = ( relTime - relTime0 ) * 1000.0;//unit: ms
    ...
    pub_func( pl_processed_rot, pub_full, msg->header.stamp );//第一个点的时间戳才是一帧开始的时间
    pub_func( pl_processed_rot, pub_surf, msg->header.stamp );//消息头的时间戳是一帧结束的时间
    pub_func( pl_processed_rot, pub_corn, msg->header.stamp );
